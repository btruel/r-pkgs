...
```{r strsplit1-commit}
#| echo = debug, comment = NA, include = FALSE, eval = create
...
```{r add-strsplit1-diff, echo = FALSE, eval = create, asis = TRUE}
...
```{cat DESCRIPTION-write, eval = create, class.source = "default"}
...
# if I provide "LICENSE" directly to readLines() below, it grabs "LICENSE"
# from this book, not from regexcite
# i.e. it bypasses the working directory that is otherwise in force
# so force the path here in a normal chunk
LICENSE_path <- normalizePath("LICENSE")
```
...
```{cat, code = readLines(LICENSE_path), class.source = "default", eval = create}
```
...
```{r commit-strsplit1-roxygen-header}
#| echo = debug, comment = NA, include = FALSE, eval = create
git_add(path("R", "strsplit1.R"))
...
#| engine.opts = list(file = path("tests", "testthat", "test-strsplit1.R"))
...
```{r commit-strsplit1-test, echo = debug, comment = NA, eval = FALSE}
git_add(test_path)
...
```{r include = debug, eval = create}
# if I provide "README.Rmd" directly to readLines() below, it looks for
# "README.Rmd" in this book, not in regexcite
# i.e. it bypasses the working directory that is otherwise in force
# so force the path here in a normal chunk
README_path <- normalizePath("README.Rmd")
```

<!-- This is a way to get the contents of a file displayed in a code-link chunk, with a copy button, but without syntax highlighting. -->

```{cat, code = readLines(README_path), class.source = "default", eval = create}
```
